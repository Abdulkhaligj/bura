...
export async function handle(request,env={}){
 ...
 const owner=async c=>{auth();if(user.role!=='admin'&&c.owner_id!==user.id){const org=await one('SELECT owner_id FROM content WHERE id=?',c.organization_id||c.id);if(org?.owner_id!==user.id)fail('Bu məlumatı idarə edə bilməzsən.',403)}};
 ...
 if(path==='/api/document'&&method==='POST'){
  ...
  const docId=d.id||id();if(d.id&&!await one('SELECT id FROM documents WHERE id=? AND user_id=?',d.id,user.id))fail('Sənəd tapılmadı.',404);
 ...
 if(itemMatch){const [,iid,action]=itemMatch,c=await content(iid),b=parse(c.body);
  ...
  if(action==='apply'){
   if(c.kind!=='opportunity')fail('Bu elan müraciət qəbul etmir.');if(b.deadline&&new Date(b.deadline+'T23:59:59+04:00').getTime()<now())fail('Müraciət müddəti bitib.');const note=str(d.note,20,3000);
   const appId=id();const result=await q('INSERT OR IGNORE INTO applications (id,user_id,item_id,note,profile,created_at,updated_at) VALUES (?,?,?,?,?,?,?)',appId,user.id,iid,note,JSON.stringify(safeUser(user)),now(),now()).run();if(!result.meta.changes)fail('Bu elana artıq müraciət etmisən.',409);return json({id:appId});
  }
  if(action==='register'){
 ...
 if(path==='/api/application'&&method==='POST'){
  ...
  if(d.status==='withdrawn'){if(a.user_id!==user.id)fail('İcazə yoxdur.',403)}else{await owner(await content(a.item_id));if(!['reviewing','interview','accepted','rejected'].includes(d.status))fail('Status düzgün deyil.');if(a.status==='withdrawn')fail('Müraciət geri götürülüb.',409)}
  await db.batch([q('UPDATE applications SET status=?,updated_at=? WHERE id=?',d.status,now(),a.id),notify(a.user_id,'Müraciətinin statusu yeniləndi.','#/profile')]);return json({ok:true});
 }
